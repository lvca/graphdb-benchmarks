...
public class ArcadeProfiler extends AbstractProfiler {
  ...
  public ArcadeProfiler(String basePath, String host, int port, String dbName, String dbUser, String dbPasswd) {
    this();
   ...
   */
  public ArcadeProfiler(String basePath, String host, String dbName, String user, String passwd) {
    ...
    try {
      return XStopWatch.profile(() -> {
        try (RemoteDatabase database = new RemoteDatabase(host, port, dbName, dbUser, dbPasswd)) {
          final String language = gremlinQuery.contains(":") ? "sqlscript" : "sql";
          ResultSet rs = database.query(language, gremlinQuery);
